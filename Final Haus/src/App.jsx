...
function Partnership() {
  const { t } = useLang();
  return (
    <section className="px-5 sm:px-8 py-14 sm:py-20" data-testid="partnership-section">
      <Reveal className="max-w-3xl mx-auto text-center">
        <h2 className="font-serif-accent italic text-[#0E0E0F] text-3xl sm:text-5xl lg:text-6xl leading-tight">
          {t.partnership.title}
        </h2>

        <div className="mt-7 sm:mt-10 rounded-3xl overflow-hidden border border-black/10 shadow-[0_20px_60px_-32px_rgba(0,0,0,0.35)]">
          <img
        ...
        <p className="mt-3 font-body text-[13px] sm:text-sm text-black/45">{t.partnership.photoCaption}</p>

        <p className="mt-5 sm:mt-7 font-display font-semibold text-xl sm:text-3xl tracking-tight leading-snug max-w-2xl mx-auto">
          {t.partnership.caption}
        </p>

        <p className="mt-4 font-body text-sm sm:text-base text-black/55 leading-relaxed max-w-xl mx-auto">
          {t.partnership.note}
        </p>
      </Reveal>
    </section>
...
function Manifesto() {
  const { t } = useLang();
  const [open, setOpen] = useState("land"); // 一进来预设展开第一个理由
            ...
            <div key={c.no} className="border-b border-black/12" data-testid={`chapter-${c.no}`}>
              ...
              >
                ...
                <span className="flex-1 min-w-0">
                  <span className="block font-serif-accent italic text-sm text-[#C6A15B]/80 leading-none mb-1">{c.no}</span>
                  <span className="block font-display font-semibold text-lg sm:text-2xl tracking-tight leading-snug text-[#0E0E0F]/90">
                    {c.title}
                  </span>
                </span>
      ...
      <div className="mt-6 grid lg:grid-cols-[1.6fr,1fr] gap-6 items-start">
        ...
        <div className="rounded-2xl bg-[#F0ECE3] border border-black/5 p-6">
          ...
          <div className="mt-6 space-y-4 text-black/70 font-body">
            ...
            <div className="flex items-center gap-2"><TrendingUp className="w-5 h-5 text-[var(--taupe-deep)]" /> {u.yield}% {t.units.roi}</div>
          </div>
          <div className="mt-6 pt-5 border-t border-black/10">
            ...
            <p className="mt-3 text-[11px] leading-snug text-black/35 font-body">{t.units.rentNote}</p>
          </div>
        </div>
      </div>
...
function CatchmentCluster({ cluster, side, studentLabel, studentWord, delay = 0 }) {
  return (
    ...
    >
      ...
      <p className="mt-2 font-display text-[11px] tracking-[0.18em] uppercase text-[var(--taupe-deep)]/75">{studentLabel}</p>
...
function RentalDemand() {
  ...
  return (
    <section id="rental-demand" className="px-5 sm:px-8 py-16 sm:py-24 max-w-6xl mx-auto scroll-mt-24" data-testid="rental-demand-section">
      ...
      <p className="mt-12 sm:mt-16 text-center font-display font-bold text-sm sm:text-base tracking-[0.18em] uppercase text-[#0E0E0F]/70">
        {r.chartTitle}
      </p>
        ...
        >
          ...
          {[-45, -135, 45, 135].map((deg) => (
            <span
              key={deg}
              aria-hidden="true"
              className="hidden lg:block absolute left-1/2 top-1/2 h-px w-[110px] bg-[#0E0E0F]/20 origin-left"
